...
if debug > 1
    ...
    println(""); print("Condition number Lup: ", cond(L_up)); println("")
    #println(""); print("Llow = ", size(L_down), " "); show(L_down); println("")
    println(""); print("Condition number Llow: ", cond(L_down)); println("")
    println(""); print("Condition number BigL: ", cond(BigL)); println("")
    println(""); print("w = ", size(x), " "); show(slf.w(x)); println("")
    println(""); print("p = ", size(x), " "); show(slf.p(x)); println("")
    println(""); print("p' = ", size(x), " "); show(slf.pp(x)); println("")
    println(""); print("V = ", size(x), " "); show(slf.V(x,inputs.q,inputs.m)); println("")
    println(""); print("gam = ", size(x), " "); show(slf.gamma(x)); println("")
...
# Requires A to be real, symmetric tridiagonal
#println("GLA.eigQR!: ", cond(BigL), ThreadsX.sort!(GenericLinearAlgebra.eigQR!(copy(BigL)), alg=ThreadsX.StableQuickSort, by = x -> sqrt(real(x)^2 + imag(x)^2)))
#P = AMGPreconditioner{RugeStuben}(copy(BigL))
#println("GLA with RugeStuben preconditioner: ", cond(BigL \ P), ThreadsX.sort!(GenericLinearAlgebra.eigvals!(BigL \ P), alg=ThreadsX.StableQuickSort, by = x -> sqrt(real(x)^2 + imag(x)^2)))
#P = AMGPreconditioner{RugeStuben}(copy(BigL))
#println("GLA with RugeStuben preconditioner: ", cond(BigL \ P), ThreadsX.sort!(GenericLinearAlgebra.eigvals!(BigL \ P), alg=ThreadsX.StableQuickSort, by = x -> sqrt(real(x)^2 + imag(x)^2)))
println("Done!")
